...
// 背景色後の画像アニメーション //
    //要素の取得とスピードの設定
    const box = $('.color-animation'),
    speed = 700;  

    //.colorboxの付いた全ての要素に対して下記の処理を行う
    box.each(function(){
    $(this).append('<div class="color-animation__bg"></div>')
    const color = $(this).find($('.color-animation__bg')),
    image = $(this).find('img');
    const counter = 0;
...
// ページトップ //
    ...
    //         $(".js-pagetop").css({ position: "absolute", bottom: footHeight + 20 }); //positionをabsoluteに変更
    //     } else { //それ以外の場合は
    //         $(".js-pagetop").css({ position: "fixed", bottom: "rem(20)" }); //固定で表示
    //     }
    // });
    // });
});
